...
export const getForm144Schema = z.object({
  ...
  insider_name: z.string().optional().describe('Filter by insider name (partial match)'),
  from: z.string().optional().describe('Start filing date in ISO 8601 format, e.g. 2026-01-01'),
  to: z.string().optional().describe('End filing date in ISO 8601 format, e.g. 2026-12-31'),
  exclude_10b5: z
    .boolean()
    .optional()
    .describe('If true, exclude pre-scheduled 10b5-1 plan filings. Recommended for signal analysis — discretionary sales only.'),
